/**
 * OMDb Third-Party API Integration (Client-Side)
 * Sends AJAX requests to the Laravel OmdbController.
 * The OMDb API key is stored in .env and never exposed to the client.
 */

window.omdbApi = (() => {
  // API URL now points to Laravel API route
  const apiUrl = "/api/omdb";
  ...
  const getMovieDetails = async (imdbId) => {
    const response = await fetch(
      `${apiUrl}/details?imdb_id=${encodeURIComponent(imdbId)}`,
      { headers: { Accept: "application/json" } }
    );
